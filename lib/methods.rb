...
def the_amnesty_headline_scraper
  ...
  doc_amnesty = Nokogiri::HTML(html_amnesty)
  headline_amnesty = "#{doc_amnesty.css('span.heading--tape').text}: #{doc_amnesty.css('p.image-headline__copy').text}"
  backup_headline = doc_amnesty.css('span.heading--tape').text
  error_headline = "Sorry, still waiting on headline from Amnesty International USA..."
  if !headline_amnesty.scan(/\w/) && backup_headline.scan(/\w/)
    backup_headline
  elsif headline_amnesty.scan(/\w/)
    headline_amnesty
  else
...
def the_hrw_headline_scraper
  ...
  if headline_hrw.scan(/\w/)
    ...
    headline_aclu = "Sorry, still waiting on headline from Human Rights Watch..."
  end
end
...
def the_splc_headline_scraper
  ...
  if headline_splc.scan(/\w/)
    ...
    headline_aclu = "Sorry, still waiting on headline from SPLC.org..."
  end
end
...
def the_aclu_url_scraper
  ...
  if aclu_url.scan(/\w/)
    ...
    headline_aclu"Sorry, waiting on article URL from ACLU.org..."
  end
end
...
def the_splc_url_scraper
  ...
  step_3 = step_2[0].children
  step_4 = step_3[1].children.text
  splc_url = step_4.scan(/https.*\w/)[0]
  if splc_url.scan(/\w/)
    splc_url
  else 
...
def scrape_executor
  puts "Welcome to WokeApp!"
  puts "Please wait while we generate today's social justice news..."
  aclu_object_maker
  amnesty_object_maker
  hrw_object_maker
  splc_object_maker
...
def root_menu
	...
	if input == "exit"
		...
		puts "Sorry, we don't recognize that input! Redirecting..."
		root_menu
	end
end	
